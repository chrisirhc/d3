d3 = function() {
  ...
  d3.layout.dag = function() {
    ...
    function initTree() {
      ...
      for (ii = 0; ii < userLinks_.length; ++ii) {
        ...
        if (typeof link.source == "number") {
          if (link.source < 0 || link.source >= userNodes_.length) throw "Invalid link at index " + ii + " (source node " + link.source + " does not exist)";
      ...
      for (ii = 0; ii < userNodes_.length; ++ii) {
        if (!userNodes_[ii].inputs.length) roots_.push(userNodes_[ii]);
      }
    }
    function initRanks() {
    ...
    function addVirtualNodes() {
      var curIndex = nodes_.length, newLinks = [], ii, jj;
      for (ii = 0; ii < links_.length; ++ii) {
        var link = links_[ii];
    ...
    function initOrder() {
      var visitingNow = roots_.slice(0), visitingNext, newLen, ii, jj;
      order_ = [];
      ...
      while (visitingNow.length > 0) {
        ...
        for (ii = 0; ii < visitingNow.length; ++ii) {
          ...
          if (!n.__visited__) {
            ...
            for (jj = 0; jj < n.outputs.length; ++jj) visitingNext.push(n.outputs[jj].target);
          }
        }
    ...
    function weightedMedianOrdinal(node, reverse) {
      ...
      for (var ii = 0; ii < adjLinks.length; ++ii) adjOrdinals.push(adjLinks[ii][prop].ordinal);
      var medianIdx = Math.floor(adjOrdinals.length / 2);
      if (!adjOrdinals.length) return null; else if (adjOrdinals.length % 2 == 1) return adjOrdinals[medianIdx]; else if (adjOrdinals.length == 2) return (adjOrdinals[0] + adjOrdinals[1]) / 2; else {
    ...
    function orderByMedian(reverse, reverseEqual) {
      ...
      var sortFn = function(a, b) {
        ...
        if (diff === 0) return reverseEqual ? a.index - b.index : b.index - a.index; else return diff;
      };
    ...
    function countCrossings(left, right) {
      var crossingCount = 0;
      for (var mm = 0; mm < left.outputs.length; ++mm) for (var nn = 0; nn < right.outputs.length; ++nn) if (left.outputs[mm].target.ordinal > right.outputs[nn].target.ordinal) crossingCount++;
      return crossingCount;
    }
    function countAllCrossings() {
      var crossingCount = 0, rr, rankOrder, ii, jj;
    ...
    function orderNodes() {
      initOrder();
      var bestOrder = saveOrder(), curCrossings = countAllCrossings(), minCrossings = curCrossings, ii, jj;
      ...
      order_ = bestOrder;
      for (ii = 0; ii < order_.length; ++ii) for (jj = 0; jj < order_[ii].length; ++jj) order_[ii][jj].ordinal = jj;
    }
    function linkPriority(link) {
      if (link.source.virtual && link.target.virtual) return 8; else if (link.source.virtual || link.target.virtual) return 2; else return 1;
    }
    ...
    function medianPosition(node, upward) {
      ...
      for (var ii = 0; ii < adjLinks.length; ++ii) adjValues.push(adjLinks[ii][adjSide][ordAxis_]);
      var medianIdx = Math.floor(adjValues.length / 2);
      if (!adjValues.length) return null; else if (adjValues.length % 2 == 1) return adjValues[medianIdx]; else return (adjValues[medianIdx - 1] + adjValues[medianIdx]) / 2;
    }
    function positionNodesByMedian(upward) {
      ...
      var sortFn = function(a, b) {
        return b[priorityProp] - a[priorityProp];
      };
      for (var ii = 1; ii < order_.length; ++ii) {
        ...
        for (var jj = 0; jj < rank.length; ++jj) {
          ...
          for (var kk = 0; kk < jj; ++kk) {
            if (rank[kk].ordinal < node.ordinal) pos = Math.max(pos, rank[kk][ordAxis_] + nodeSeparation_ * (node.ordinal - rank[kk].ordinal)); else pos = Math.min(pos, rank[kk][ordAxis_] - nodeSeparation_ * (rank[kk].ordinal - node.ordinal));
          }
          node[ordAxis_] = pos;
        }
      }
    ...
    function normalizePositions() {
      var minPos = Infinity, curMargin = margin_[(ordIdx_ + 3) % 4], ii;
      for (ii = 0; ii < nodes_.length; ++ii) {
    ...
    function positionNodes() {
      var rankSpans = [], rankSum = 0, ii, jj;
      for (ii = 0; ii < order_.length - 1; ++ii) {
        rankSpans[ii] = 1;
        for (jj = 0; jj < order_[ii].length; ++jj) if (order_[ii][jj].rankSpan) rankSpans[ii] = Math.max(rankSpans[ii], order_[ii][jj].rankSpan);
        rankSum += rankSpans[ii];
      }
      var usableSpace = size_[rankIdx_] - margin_[rankIdx_ + 1] - margin_[(rankIdx_ + 3) % 4], rankPos = [], nextRankPos = 0;
      rankSize_ = usableSpace / rankSum;
      for (ii = 0; ii < order_.length; ++ii) {
        rankPos[ii] = orientation_ == "rl" || orientation_ == "bt" ? size_[rankIdx_] - margin_[rankIdx_ + 1] - nextRankPos : nextRankPos;
      ...
      for (ii = 0; ii < nodes_.length; ++ii) {
        ...
        for (jj = 0; jj < node.inputs.length; ++jj) node.priorityUp += linkPriority(node.inputs[jj]);
        for (jj = 0; jj < node.outputs.length; ++jj) node.priorityDown += linkPriority(node.outputs[jj]);
      }
      var curScore = scorePosition(), minScore = curScore, bestPosition = saveNodePosition();
      for (ii = 0; ii < 8; ++ii) {
    ...
    self_.paths = function() {
      return userLinks_.map(function(link) {
        return link.path;
      });
    };
